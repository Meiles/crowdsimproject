...
void createMenu() {
    ...
    glutAddMenuEntry("Toggle Environment",4);
             ...
             glutAddMenuEntry("Add Single Agent", 6);
     glutAddMenuEntry("Add 5 Agents", 7);
     glutAddMenuEntry("Add 10 Agents", 8);
    glutAddMenuEntry("Add 25 Agents", 9);
    //glutAddMenuEntry("Upload input.txt", 10);
  ...
  if( cameraMode == 1 /*top*/ && editEnvironment && button == GLUT_LEFT_BUTTON && state == GLUT_DOWN ) {
    ...
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        if( !mousePressed ) {
            homeX = x;
            homeY = y;
            double x_env = x - windowW / 2.0;
            double y_env = (windowH / 2.0 - y);

            //Using this and passing in agents to add is selected from the menu
            gSim.addAgent(agentsToAdd, x_env, y_env );
